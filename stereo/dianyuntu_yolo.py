...
def getRectifyTransform(height, width, config):
    ...
    # 这个函数用于计算无畸变和修成转换关系
    # 输出左图和右图的X和Y坐标的重映射参数
    map1x, map1y = cv2.initUndistortRectifyMap(left_K, left_distortion, R1, P1, (width, height),
...
def rectifyImage(image1, image2, map1x, map1y, map2x, map2y):
    ...
    rectifyed_img1 = cv2.remap(image1, map1x, map1y, cv2.INTER_LANCZOS4, cv2.BORDER_CONSTANT,
                               0)  # cv2.INTER_LANCZOS4, cv2.BORDER_CONSTANT, 0
...
if __name__ == '__main__':
    ...
    def onMouse(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            print('点 (%d, %d) 的三维坐标 (x:%.3fm, y:%.3fm, z:%.3fm)' % (
            x, y, points_3d[y, x, 0] / 1000, points_3d[y, x, 1] / 1000, points_3d[y, x, 2] / 1000))
